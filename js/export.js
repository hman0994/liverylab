 ...
 *  - editor.getExportRenderState(size) from js/editor.js
 */

function formatExportFallbackReason(reason) {
  const fallbackReason = typeof reason === 'string' && reason ? reason : 'unsupported-document';
  return fallbackReason.replace(/-/g, ' ');
}

async function resolveExportRenderState(editor, outputSize) {
  if (editor && typeof editor.getExportRenderState === 'function') {
    return editor.getExportRenderState(outputSize);
  }
...
async function exportPNG(editor, outputSize = 2048, carFolder = '') {
  ...
  const renderState = await resolveExportRenderState(editor, outputSize);
  const imageData = renderState.imageData;
  const targetFolder = carFolder || 'your_car_folder';

  if (renderState.fallbackUsed) {
    const unsupportedCount = Array.isArray(renderState.unsupportedLayerIds)
      ? renderState.unsupportedLayerIds.length
      : 0;
    const reason = formatExportFallbackReason(renderState.fallbackReason);
    const details = unsupportedCount > 0
      ? ` (${unsupportedCount} unsupported layer${unsupportedCount === 1 ? '' : 's'})`
      : '';
    showToast(`Export fallback active: ${reason}${details}.`, 'warning');
  }
