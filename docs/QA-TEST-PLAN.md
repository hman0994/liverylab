...
Use this plan together with [MANUAL-TEST-CHECKLIST.md](./QA/MANUAL-TEST-CHECKLIST.md).
...
1. Smoke pass.
2. Full regression pass from [MANUAL-TEST-CHECKLIST.md](./QA/MANUAL-TEST-CHECKLIST.md).
...
- Core tools: select, brush, eraser, fill, rectangle, circle, line, gradient, text
...
Run the full checklist in [MANUAL-TEST-CHECKLIST.md](./QA/MANUAL-TEST-CHECKLIST.md).
...
2. Run the minimum merge checklist in [MANUAL-TEST-CHECKLIST.md](./QA/MANUAL-TEST-CHECKLIST.md).
...
| Agent 04 - History And Commands | Broken undo grouping, duplicate history entries, redo corruption, or transition clashes with snapshot history | Run Undo and Redo Regression, then targeted checks for brush, fill, layer visibility toggle, object move, and template/document load boundaries; verify a new action clears redo as expected | Undo/redo order changes unexpectedly, redo stack survives when it should clear, or partial command history and snapshot history double-record the same action |
...
2. The full checklist in [MANUAL-TEST-CHECKLIST.md](./QA/MANUAL-TEST-CHECKLIST.md) passes in at least one primary Windows browser.
