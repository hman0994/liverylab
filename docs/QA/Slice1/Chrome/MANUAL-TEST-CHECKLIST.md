...
- [PASS W/I] Draw on the active paint layer.
- [FAIL] Confirm the result appears where expected.
- [PASS] Undo and redo the stroke.
When painting on a layer other than base paint, the brush stroke applies to the basepaint, and also shifts the position of the selected layer. Behavior should function where the brush can paint on the active layer, and no shifting of layers occurs.

### Eraser

- [PASS] Select Eraser with `E`.
- [PASS W/I] Erase part of a painted area.
- [FAIL] Confirm only the intended painted area is removed.
- [PASS] Undo and redo the erase.
When erasing on a layer other than base paint, the erase stroke does not apply, and also shifts the position of the selected layer. Behavior should function where the brush can erase on the active layer, and no shifting of layers occurs.

### Fill

- [PASS] Select Fill with `F`.
- [PASS W/I] Fill the active area or layer target.
- [FAIL] Confirm the color change applies as expected.
- [PASS] Undo and redo the fill.
After filling, the object should be selected.
...
- [FAIL] Duplicate a selected object/layer and confirm a new editable item appears.
