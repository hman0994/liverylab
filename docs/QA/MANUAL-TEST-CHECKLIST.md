...
- [] For a render/export slice, complete local preflight first: confirm every required render/export matrix row has a planned artifact target and confirm at least one unsupported fallback case is pre-selected as a fallback-only check.
- [] For a render/export slice, do not mark pass from local preflight only; attach browser evidence artifacts for each required render/export row before making a pass/block decision.
...
- [] For the current committed mixed-content render/export slice, record one supported viewport-vs-export comparison note that includes template, guide, and spec-map policy checks plus at least one committed text or supported shape layer.
- [] For the current committed mixed-content render/export slice, record one supported committed image-backed `multiply` or `screen` note confirming the document still reports shared viewport-base support after commit and that PNG/TGA output matches the visible committed canvas result.
- [] On that same committed `multiply` or `screen` document, select the blended layer and confirm pointer-transform or keyboard-nudge live overlay falls back cleanly to the normal Fabric scene instead of trying to preview the blend above the mounted shared base.
- [] For the current committed mixed-content render/export slice, record one unsupported-content fallback note confirming the document still exports through the legacy path as expected and does not claim a shared viewport parity artifact.
- [] For the current committed mixed-content render/export slice, record one unsupported-blend fallback note confirming a document with a committed blend outside the current image-backed `source-over` or `destination-out` or `multiply` or `screen` subset, or any non-`source-over` committed text or shape blend, still stays on the legacy viewport/export path instead of claiming shared-base support.
- [] For the current live viewport-base cutover slice, at one non-1.0 zoom run `editor.getViewportBaseRenderState()` on a supported committed mixed-content document and confirm it reports `supported: true`, current-zoom target dimensions, and composed committed layer ids, and confirm the mounted shared base visibly replaces the lower-canvas committed scene on that document; then confirm an unsupported committed-content document still reports `supported: false`.
- [] For the current live viewport-base cutover slice, pass the same caller-supplied canvas into `editor.getViewportBaseRenderState()` for two consecutive supported committed mixed-content renders and confirm the second result does not inherit stale transform, opacity, or blend state from the first draw.
- [] For the current live viewport-base cutover slice, pointer-drag one supported single selected committed layer in select mode and confirm the mounted shared base stays active while only the transformed object renders above it, then confirm the committed scene resyncs cleanly after the interaction ends.
- [] For the current live viewport-base cutover slice, run one short keyboard nudge burst on one selected supported committed layer and confirm the mounted shared base stays active while only the nudged object renders live above it, then confirm the committed scene resyncs cleanly after the grouped commit settles.
- [] For the current live viewport-base cutover slice, immediately click a different object after one supported keyboard nudge burst and confirm the temporary keyboard live overlay tears down cleanly instead of leaving the viewport briefly stuck on the previous live object.
- [] For the current live viewport-base cutover slice, run one keyboard nudge burst on an unsupported selected target such as multi-select or another unsupported live case and confirm the editor still falls back to the normal Fabric scene during the burst, then resumes the mounted shared base cleanly afterward.
- [] For the current live viewport-base cutover slice, place or edit one text layer on a supported committed mixed-content document and confirm the editor stays on the normal Fabric scene only while text editing is active, then resumes the mounted shared base after editing exits without dropping the document to legacy solely because text now exists.
- [] For the current live viewport-base cutover slice, start one rect or circle or line or gradient preview and confirm the transient preview stays visible while the mounted shared base remains active underneath, then cancel once and commit once and confirm the committed scene resyncs cleanly afterward while the committed supported shape remains on the shared path instead of falling back solely because the document is no longer raster-only.
- [] For the current live viewport-base cutover slice, run one transaction-backed layer mutation such as visibility toggle or one-step reorder on a supported committed mixed-content document and confirm the mounted shared base resyncs immediately after the commit.
- [] For the current live viewport-base cutover slice, start one Brush or Eraser stroke on a supported committed mixed-content document where the active target is a qualifying translated or axis-aligned resized image-backed raster layer and confirm the mounted shared base stays active underneath the live stroke preview, then confirm the committed scene resyncs cleanly after the stroke finishes.
- [] For the current committed-raster fill follow-up, save a project after filling both one strict replay-safe image-backed raster layer and one transformed or committed supported-blend `shared-surface-commit-only` raster layer, reload it, and confirm both filled layers remain editable and export correctly.
- [] For the current committed-raster brush or eraser follow-up, save a project after one supported edge or smaller-layer stroke edit plus one transformed `shared-surface-commit-only` stroke edit, reload it, and confirm the edited raster layers remain aligned, editable, and exportable.
- [] For the current live viewport-base cutover slice, on one rotated or skewed or negative-scale or otherwise unsupported raster free-draw target, confirm Brush or Eraser still stays on the normal Fabric fallback path and does not claim mounted shared-base support.
- [] For the current live viewport-base cutover slice, with Brush or Eraser still selected after one undo or redo on a qualifying translated or axis-aligned resized image-backed raster target, confirm the mounted shared base is available again when the restored target still meets the current cropped shared-surface commit boundary.
...
- [] For the current history module, verify repeated arrow-key nudges on one selected object collapse into one undo unit after the idle boundary and still clear redo after a divergent action.
- [] For the current pointer-transform history follow-up, drag one selected uploaded image, one text object, and one shape, confirm each drag move maps to one undo unit, and confirm redo clears after a divergent action.
- [] For the current pointer-transform history follow-up, after one translate-only drag confirm `editor.getHistoryDebugState()` reports `mode: 'pointer-move'` for that latest committed transform entry.
- [] For the current pointer-transform history follow-up, on single selected objects run one rotate, one scale or resize, and when the current controls allow it one skew, flip, or mixed transform, confirm each action maps to one undo unit, and confirm redo clears after a divergent action.
- [] For the current pointer-transform history follow-up, after one non-translate single-object transform confirm `editor.getHistoryDebugState()` reports `mode: 'pointer-transform'` plus the expected `kind` and `transformKinds` metadata for that latest committed transform entry.
- [] For the current pointer-transform history follow-up, if you can create a multi-select or grouped transform, perform one such transform and confirm it stays on the fallback path without double-recording through the explicit pointer-transform seam.
- [] For the current async paint barrier follow-up, use one known awaited stroke or fill path, immediately attempt Undo or Redo plus one document-boundary action such as project load or custom template load, and confirm those later actions are ignored while the paint commit is still in flight, the template modal stays open on blocked template loads, project load can be retried after the `document.stroke.commit` or `document.fill.apply` entry has finished committing without selecting the file again, and blocked background-colour edits snap back to the real document value.
- [] For the current async paint replay follow-up, use one replay-backed Brush or Eraser or active paint-layer Fill case, trigger Undo or Redo, immediately trigger the same history action again before the first replay visually settles, and confirm the second request is ignored until the first replay finishes instead of skipping to the wrong snapshot or bouncing back through an intermediate fallback load.
- [] For the current targeted replay follow-up, commit one replayable action with no selected-object anchor such as template-opacity while nothing is selected, click a different fillable object afterward without committing a new history entry, then undo and redo and confirm the replayed null-selection state clears the visible selection; immediately switch to `Fill`, click inside that same later object, and confirm the fill does not treat it as a retained selected-object target unless you explicitly reselect it first.
- [] For the current commit-only paint replay follow-up, commit one transformed or committed supported-blend existing-raster Brush/Eraser plus one existing paint-layer Fill and one retained-raster Fill that each report `commitPath: 'shared-surface-commit-only'`, then undo once and redo once for each and confirm the same target layer is restored directly in one step when the current target still matches the documented committed shared-raster boundary.
- [] For the current layer-add replay slice, upload one image, duplicate one selected layer, place one text layer, and commit one rect or circle or line or gradient preview, confirm each action maps to one undo unit, confirm redo returns the inserted layer selected through the targeted replay path, and confirm `editor.getHistoryDebugState()` shows the latest committed `document.layer.add` entry with inserted layer id, insert index, `upload-image`, `duplicate-selected`, `text-insert`, `rect-preview-finalize`, `circle-preview-finalize`, `line-preview-finalize`, or `gradient-preview-finalize` source metadata, and matching after-selection anchors.
- [] For the current layer-remove replay slice, delete one single selected layer and one layer through the panel row action, confirm each action maps to one undo unit, confirm undo restores the same removed layer at the same stack position and redo removes that same layer again through the targeted replay path, and confirm unsupported multi-delete or ambiguous structural cases still stay on the normal snapshot fallback boundary.
- [] For the current document-boundary slice, verify project load and PSD import both clear redo, stay non-undoable across the boundary, expose the expected checkpoint-only debug entry through `editor.getHistoryDebugState()`, and still allow one export immediately after the boundary.
- [] If the slice changes tool or workspace behavior, verify toolbar state, shortcut state, inspector state, layer-panel state, and panel collapse state stay aligned.
